<script>
  ...
  import { fetchApps, fetchProfile, getAppSlug } from "$lib/nostr.js";
  import { authStore } from "$lib/stores/auth.js";

  // Franzap npub to fetch real data
  ...
  const franzapPubkey = npubToHex(FRANZAP_NPUB);
  ...
  onMount(async () => {
    if (franzapPubkey) {
      try {
        const fetchedApps = await fetchApps({ authors: [franzapPubkey], limit: 10 });
        apps = fetchedApps;
        profile = await fetchProfile(franzapPubkey);
      } catch (e) {
        console.error("Failed to fetch franzap data:", e);
      }
    }
    loading = false;
  });
  ...
  $: chartData = {
    ...
    apps: generateAppChartData(apps.length || 3)
  };

  const chartHeight = 100;
  const chartPadding = { left: 0, right: 16, top: 10, bottom: 10 };
  
  $: maxDownloads = Math.max(...(chartData.downloads || [100])) * 1.2;
  $: maxZaps = Math.max(...(chartData.zaps || [1000])) * 1.2;
  ...
  function generateSmoothPath(data, maxValue, chartWidth, height) {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      const cp1x = prev.x + (curr.x - (points[i - 2]?.x || prev.x)) * tensionFactor;
      const cp1y = prev.y + (curr.y - (points[i - 2]?.y || prev.y)) * tensionFactor;
  ...
  let downloadsChartWidth = 400;
  let zapsChartWidth = 400;
</script>
      ...
      <aside class="sidebar-panel">
        ...
        <div class="sidebar-section">
          ...
          {:else if apps.length === 0}
            <div class="sidebar-loading">No apps</div>
          {:else}
            <div class="sidebar-apps">
              {#each apps as app}
                <a href="/apps/{getAppSlug(app.pubkey, app.dTag)}" class="sidebar-app-item">
                  <div class="sidebar-app-icon">
                    <AppPic iconUrl={app.icon} name={app.name} size={24} />
                  </div>
